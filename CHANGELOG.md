...
- A `BoundsContainer` trait (explicit opt-in) marking bounds containers writable by `Vecs` and `Strings`, and a `BoundsBorrow` composite for types whose borrowed form answers `Bounds` queries
