...
- A `Lengths` trait for the bounds of `Vecs` and `Strings`, unifying the read side of list extents: `bounds(index)` reports each list's extent ("select"), `rank(offset)` reports the list containing a value position, `extent(range)` the positions spanned by a range of lists, `extents(ranges)` its bulk in-place form over ascending disjoint ranges (one cursor pass on the bitvec containers), and `total()` the summed lengths. Implemented by `Uppers` (the new default lengths container), `Strided`, `Fixeds`, and (read-only) `Vec<u64>`/`[u64]` for raw cumulative-offset arrays
- A `LengthsContainer` trait (explicit opt-in) marking lengths containers writable by `Vecs` and `Strings`, with provided `seal(upper)` (conclude the current list at an absolute position; `Uppers` records it directly) and `extend_with_extent(other, range, extent)` (extend by a range whose extent the caller already computed, sparing bit-packed implementors a re-derivation costing a select or two per call; `Vecs`/`Strings` pass down the extent they compute for the values copy); and a `LengthsBorrow` composite for types whose borrowed form answers `Lengths` queries
- Two `RankSelect`-backed lengths containers: `NeverEmpty` for non-empty lists (one bit per value, set at each list's last value) and `MaybeEmpty` for possibly-empty lists (unary: a zero per value, then a one). Their `rank` is a bitvector rank rather than a search, and their `extend_from_self` splices bit ranges word-at-a-time and catches up the count summaries, rather than re-encoding via integers
...
- `Strided` and `Fixeds` `Index`/`Push` now traffic in list lengths rather than absolute upper bounds, per the `Lengths` contract; `Strided::rank` divides while strided, and the `Deref`-based `Strided::bounds` helper is replaced by the `Lengths` implementation
- `Strides` is renamed `Strided` -- it is a strided representation of lengths, not a container of strides -- and adopts the `Length` contract. A deprecated `Strides` remains with its original absolute-uppers meaning: reads (including decode paths over persisted data) keep compiling and stay correct, while uses warn and writes through the new machinery do not compile
- `Strided<BC>` is generic over its spill: any `LengthsContainer` works (default `Uppers`, but e.g. `Strided<MaybeEmpty>` composes the stride fast path with bit-packed overflow). The spill stores the post-stride lists' bounds *relative to the end of the strided prefix* rather than absolute offsets — a serialized-format change for spilled `Strided` data — and `Strided::extend_from_self` bulk-extends conforming strided heads and delegates spill-to-spill, so the spill's own bulk extension (memcpy, bit splice) is used
...
- `FromBytes::validate` implementations for `Fixeds`, `Strided`, `Empties`, `Bools`, and `Discriminant`; `Stash::try_from_bytes` as the validated constructor ([#85](https://github.com/frankmcsherry/columnar/pull/85))
...
- `Strided::pop` for unsealing lists at merge boundaries ([#98](https://github.com/frankmcsherry/columnar/pull/98))
